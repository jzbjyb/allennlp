...
if __name__ == '__main__':
    ...
    parser.add_argument('--inp', type=str, help='input conll and extraction file (in the same order).',
                        required=True)
    ...
    with open(ext_file, 'r') as fin, open(args.out, 'w') as fout:
        for i, l in enumerate(fin):
            ...
            fout.write('\t'.join(l))
            count += 1
    assert count == len(scores), 'number of scores not equal to number of extractions'
